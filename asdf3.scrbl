...
with each layer tuned towards the performance of the below layer,
in a designed centered around the machine, not the human.
Software is written in low-level languages each centered
around the implementation details of each layer,
...
@subsection{Meet me in the middle}
...
T diagrams: make the underlying category explicit and subject to manipulation, instead of just the arrow from left to right.
